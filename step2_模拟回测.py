...
from factor_backtest.sim_backtest import run_simulation


def main() -> None:
    """Step2 入口：读取同一份 YAML 配置并执行真实账户模拟回测。"""
    # Step1：解析命令行参数，获取唯一 YAML 配置文件路径。
    default_config = PROJECT_ROOT_PATH / "config" / "config.yaml"
    parser = argparse.ArgumentParser(description="YAML 驱动的真实账户模拟回测")
    parser.add_argument("--config", default=default_config, help="YAML 配置路径，默认使用 config/config.yaml")
    args = parser.parse_args()

    # Step2：加载 YAML 配置；模拟回测会使用 paths / factors / analysis / simulation 模块。
    config = BacktestConfig.load(args.config)

    # Step3：执行真实账户模拟，生成资金曲线、交易明细、选股结果和 HTML 报告。
    results = run_simulation(config)

    # Step4：通过项目统一 logger 输出结果位置，避免使用 print。
    logger.debug(f"[step2] 完成 {len(results)} 个因子模拟回测，输出目录：{config.paths.output_folder / 'step2_simulation'}")
