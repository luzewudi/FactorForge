...
# 中文本地化报告需要图表也能正确显示中文；这里优先选择 Windows 常见中文字体。
...
_DEFAULT_FONT = next(
    (font for font in _CJK_FONT_CANDIDATES if font in _AVAILABLE_FONTS),
    "DejaVu Sans",
)
try:
    _plt.rcParams["font.family"] = "sans-serif"
    _plt.rcParams["font.sans-serif"] = _CJK_FONT_CANDIDATES
    _plt.rcParams["axes.unicode_minus"] = False
except (KeyError, ValueError, OSError):
    pass


def _resolve_font(fontname):
    """把上游默认 Arial 映射到当前环境可用的中文字体，避免图表中文缺字。"""
    if fontname in {None, "", "Arial"}:
...
_sns.set_theme(
    ...
    rc={
        ...
        "font.sans-serif": _CJK_FONT_CANDIDATES,
        "axes.unicode_minus": False,
    },
)

# seaborn 会重写部分 matplotlib 字体配置；主题设置后再兜底写一次。
try:
    _plt.rcParams["font.family"] = "sans-serif"
    _plt.rcParams["font.sans-serif"] = _CJK_FONT_CANDIDATES
    _plt.rcParams["axes.unicode_minus"] = False
...
def _get_colors(grayscale):
    ...
    -------
    tuple
        (colors, line_style, alpha) - 配色方案、线型和透明度值
    """
...
def plot_returns_bars(
    ...
    """
    fontname = _resolve_font(fontname)

    # 验证波动率匹配的要求
    if match_volatility and benchmark is None:
        raise ValueError("match_volatility 需要传入 benchmark。")
